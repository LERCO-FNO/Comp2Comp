...
for root, _, files in os.walk(in_directory):
    ...
    for file in files:
        ...
        ds = pydicom.dcmread(Path(root, file))
        if filter_files(ds):
            continue
        ...
        if not ds.SeriesInstanceUID in series_paths:
            series_paths[ds.SeriesInstanceUID] = []
            
        series_paths[ds.SeriesInstanceUID].append(old_path)
    ...
    for series_uid, paths in series_paths.items():
        ...
        print(f"moved {series_uid}: {len(paths)} files to {series_dir}")
